#!/usr/bin/env bash
# Stage-1b OM axis spike (ADR-007 §Stage 1b; P1.04.5 + P1.05).
#
...
#
# Usage:
#   bash scripts/repro/stage1_om_stage1b.sh <path/to/run.jsonl>
#
# Exit codes:
...
if [[ -z "${JSONL_PATH}" ]]; then
    echo "Usage: bash scripts/repro/stage1_om_stage1b.sh <path/to/run.jsonl>" >&2
    exit 2
fi
if [[ ! -f "${JSONL_PATH}" ]]; then
    echo "stage1_om_stage1b: JSONL not found: ${JSONL_PATH}" >&2
    exit 2
fi
...
if [[ -z "${FINAL_SUMMARY}" ]]; then
    echo "stage1_om_stage1b: no safety_telemetry_final event in ${JSONL_PATH}" >&2
...
if [[ "${SAFETY_ENABLED}" == "false" ]]; then
    echo "stage1_om_stage1b: safety disabled by operator override (cfg.safety.enabled=false); gate skipped."
    exit 0
fi
...
if awk -v l="${LAMBDA_STEADY_STATE}" -v t="${THRESHOLD_VALUE}" 'BEGIN { exit !(l >= t) }'; then
    echo "stage1_om_stage1b: FAIL predicate A (saturation guard)" >&2
...
if awk -v m="${LAMBDA_MEAN}" 'BEGIN { exit !(m > 1e-6) }'; then
    ...
    if awk -v v="${LAMBDA_VAR}" 'BEGIN { exit !(v <= 1e-12) }'; then
        echo "stage1_om_stage1b: FAIL predicate B (λ adapted but stuck)" >&2
...
echo "stage1_om_stage1b: PASS audit gate (predicates A + B)."
