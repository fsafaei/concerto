#!/usr/bin/env bash
# Stage-1b OM axis audit-gate hook (ADR-007 §Stage 1b; P1.04.5).
#
# Twin of scripts/repro/stage1_as_stage1b_audit_gate.sh — same
# predicates, OM-specific exit-code messages. Runs after
...
#
# Usage:
#   bash scripts/repro/stage1_om_stage1b_audit_gate.sh <path/to/run.jsonl>
#
# Exit codes:
...
if [[ -z "${JSONL_PATH}" ]]; then
    echo "Usage: bash scripts/repro/stage1_om_stage1b_audit_gate.sh <path/to/run.jsonl>" >&2
    exit 2
fi
if [[ ! -f "${JSONL_PATH}" ]]; then
    echo "stage1_om_stage1b_audit_gate: JSONL not found: ${JSONL_PATH}" >&2
    exit 2
fi
...
if [[ -z "${FINAL_SUMMARY}" ]]; then
    echo "stage1_om_stage1b_audit_gate: no safety_telemetry_final event in ${JSONL_PATH}" >&2
...
if [[ "${SAFETY_ENABLED}" == "false" ]]; then
    echo "stage1_om_stage1b_audit_gate: safety disabled by operator override (cfg.safety.enabled=false); gate skipped."
    exit 0
fi
...
echo "stage1_om_stage1b_audit_gate: PASS audit gate (predicates A + B)."
