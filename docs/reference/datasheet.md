...
1. **`chamber-bench-partner-sets`** — the versioned partner zoo:
   public members' parameter files and checkpoints, per-member cards,
   and behavioural fingerprints. Private members appear as identity
   hashes only.
2. **`chamber-bench-leaderboard-bundles`** — the result bundles
   behind every leaderboard row, the checkpoints the learned rows
   load, and the admission-report archives behind each task's
   verdict.
3. **`chamber-bench-reference-trajectories`** — the fingerprint probe
   episodes and the power-pilot bundles: the evidence behind partner
   identities and the campaigns' sample-size choices.
