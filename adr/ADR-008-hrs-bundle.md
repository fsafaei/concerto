...
## Decision
_pending review — customer-discovery synthesis (primary decision input) is not yet available, and ADR-007 rev 3 Phase-0 staged axis spikes (secondary decision input) have not been run; see Rationale for what the current reading evidence does and does not resolve. **Working recommendation for the Month-3 review (not yet locked):** Option D — bind the HRS bundle to the 3 highest-information surviving axes from ADR-007 rev 3's staged spikes, with default ordering CM > PF > CR > SA > OM > AS for headline reporting; if all six axes survive, the default bundle is **CM × PF × CR** (latency / drop within CM; partner-familiarity; control-rate ratio); if Stage 3 PF fails, fall back to Option B (latency × drop × degraded-partner) using the F < N/4 formula from note 39; if Stage 2 CR fails, fall back to Option A (latency × drop × partner-familiarity)._
