...
**v1.0 right-sizing (2026-07-05 amendment; per ADR-027).** For CHAMBER-Bench v1.0 the zoo is scoped **per admitted task** (ADR-027 §Tier ladder), not per axis: each admitted task carries a partner set of **~10–14 members across two strata** — (i) a **parameterized scripted family** (the task's matched-reference controller with systematically varied parameters), and (ii) **frozen learned checkpoints, ≥3 seeds × 2 skill tiers**, sourced as by-products of the ADR-011 baseline training rather than as a separate training campaign. The **VLA stratum is exercised via this ADR's own §Reversibility exit ramp** and lands in ADR-027's Tier 3 (deferred candidates), not in a v1.0 partner set. The **70/30 public/private split is realized at partner-set level**: private members are reserved for leaderboard verification and are never shipped, satisfying the §3.5 requirement without a per-stratum bookkeeping layer. The full **144-partner plan (3 strata × 16 seeds × 3 checkpoints) is recorded as the superseded aspiration**: it was designed for the six-axis Phase-1 shape — a single benchmark task family whose PF axis would consume zoo diversity at scale — and ADR-026 ended that shape; carrying 144 partners across per-task admission would spend the training budget on diversity no admitted task's evaluation consumes. MEP-entropy admission and FCP-style checkpoint diversity remain the construction discipline *within* the learned stratum at its reduced size. See §Revision history 2026-07-05.

**Direct partner-pose observation does not violate the black-box-policy contract (2026-05-21 / P1.05.8; closes PR #182's Surface 6).** The frozen-partner contract is about *policy* access: the ego does not see the partner's weights, reward function, or training data, and the `_FORBIDDEN_ATTRS` shield on `chamber.partners.interface.PartnerBase` enforces this at the code level (`partner.named_parameters` raises `AttributeError`; the `EgoPPOTrainer._assert_partner_is_frozen` gate refuses any `requires_grad=True` parameter at construction time). It is **not** a contract that the partner's physical pose is invisible to the ego. A real robot's cameras / LIDAR / proximity sensors see other robots' poses; observing the partner's joint state in simulation is the same affordance, not policy access. The named AHT precedents (Liu 2024 RSS, COHERENT, Huriot-Sibai 2025) all let the ego observe partner state. Concretely: the Stage-1b AS / OM trainers (`EgoPPOTrainer` reading the `state` Box composed by `chamber.envs.stage1_obs_filter.Stage1ASStateSynthesizer`) directly observe the partner's `concat(qpos, qvel)` in the ego's input vector; the partner's *policy* remains frozen and black-box. The corresponding Stage-2 CM axis (where bandwidth / latency / drop on the ADR-003 comm channel is the axis under test) is where partner-pose mediation through ADR-003 becomes load-bearing; Stage-1b's direct-observation path is not in tension with that — the two axes measure different things (Stage-1b AS / OM measures heterogeneity of embodiment / observation modality; Stage-2 CM measures channel degradation). Documented nuance: with direct observation the ego can in principle learn partner-specific behaviour, which slightly weakens the ad-hoc claim. The conformal-CBF λ re-init on partner swap (ADR-004 §Risks #2) is the existing safeguard against that drift. The widening lands in ADR-007 §Stage 1b Rev 12 / P1.05.8; full rationale at `spikes/results/stage1-failure-investigation/2026-05-20/CONSULTATION_BRIEF.md` §4 Q2.
...
- 2026-07-05 amendment (per ADR-027 CHAMBER-Bench v1.0): §Decision
  extended with the v1.0 right-sizing paragraph — per-admitted-task
  partner sets of ~10–14 across two strata (parameterized scripted
  family; frozen learned checkpoints ≥3 seeds × 2 skill tiers as
  baseline-training by-products), VLA stratum deferred to Tier 3 via
  the §Reversibility exit ramp, 70/30 split realized at
  partner-set level, and the 144-partner plan recorded as the
  superseded aspiration (designed for the six-axis Phase-1 shape that
  ADR-026 ended). The black-box contract this zoo serves is now
  formalized in ADR-018. Status stays **Accepted (2026-05-13)**; no
  prior Decision content is altered.
- 2026-05-21 amendment (P1.05.8 / closes PR #182's Surface 6): §Decision extended with the direct-observation-of-partner-pose paragraph clarifying that the frozen-partner contract is about *policy* access (weights / reward / training data; enforced by `chamber.partners.interface._FORBIDDEN_ATTRS` and `chamber.benchmarks.ego_ppo_trainer._assert_partner_is_frozen`), not pose visibility. The Stage-1b AS trainer's ego observes the partner's `concat(qpos, qvel)` directly via the widened `chamber.envs.stage1_obs_filter.Stage1ASStateSynthesizer` (ADR-007 §Stage 1b Rev 12). Documented nuance: with direct observation the ego can in principle learn partner-specific behaviour, slightly weakening the ad-hoc claim; the conformal-CBF λ re-init on partner swap (ADR-004 §Risks #2) is the existing safeguard. Stage-2 CM is the slice where partner-pose-through-ADR-003-comm-channel becomes load-bearing; AS / OM and CM measure different axes and the two paths do not conflict. Cross-ref: `spikes/results/stage1-failure-investigation/2026-05-20/CONSULTATION_BRIEF.md` §4 Q2. Status stays **Accepted (2026-05-13)**.
