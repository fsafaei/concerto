...
- [What CHAMBER-Bench v1.0 contains](#what-chamber-bench-v10-contains)
...
## What CHAMBER-Bench v1.0 contains

The suite composition is pinned by the `chamber.tasks` registry
([ADR-027](adr/ADR-027-chamber-bench-v1-protocol.md) &sect;Tier ladder /
&sect;Versioning): every task carries exactly one tier, an admission
status, and a per-axis validity row backed by committed evidence. The
table below is generated from the registry (`chamber-eval manifest`
emits the machine-readable form); CI fails if it drifts. `ADMITTED`
statuses are earned, never asserted: `chamber-eval admission` executes
the ADR-027 A1/A2/A3 admission protocol under a tag-locked
pre-registration and writes the immutable report archive the status
cites (see `spikes/results/admission/`).
...
Per-task tables only — no scalar composite in v1.0
([ADR-027](adr/ADR-027-chamber-bench-v1-protocol.md) &sect;Reporting
rules). Every row is recomputed from committed result bundles that
pass the full `chamber-eval verify` check table at render time
(unverifiable entries refuse the render), under the tag-locked
campaign pre-registration. REF-SCRIPT rows are the *oracle reference*
(never a baseline); B-JOINT rows are the *non-AHT upper anchor*,
evaluated as the pair they trained as. The block below is generated
by `scripts/render_leaderboard_table.py`; CI fails if it drifts.
...
|---|---|---|---|---|---|---|
| REF-SCRIPT | *oracle reference* | 1.000 [1.000, 1.000] | 102.0 | 1.00 (imp_stiff_low) … 1.00 (imp_stiff_low) | 0, 1, 2, 3, 4 | `spikes/results/benchmark/cocarry-v1/ref-script-2026-07-05` |
| B-RND |  | 0.000 [0.000, 0.000] | 277.8 | 0.00 (imp_stiff_low) … 0.00 (imp_blend_b) | 0, 1, 2, 3, 4 | `spikes/results/benchmark/cocarry-v1/b-rnd-2026-07-05` |
| B-STAT |  | 0.492 [0.396, 0.591] | 101.4 | 0.16 (imp_lag_bounded) … 0.66 (imp_blend_b) | 0, 1, 2, 3, 4 | `spikes/results/benchmark/cocarry-v1/b-stat-2026-07-05` |
